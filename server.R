...
shinyServer(function(input, output) {
  ...
  output$ycorrect <- renderPlot({
    ...
    dframeAdj <- cbind(rbind(dframe, dframe), 
                       with(dframe, rbind(data.frame(seg.ystart=y - ell/2, 
                                                     seg.yend=y + ell/2, type="Segment"), 
                                          data.frame(seg.ystart=ell/2, 
                                                     seg.yend=-ell/2, type="Adjustment"))), 
                       adj="Original Data")
    
    if(corr==4){
      dframeAdj1 <- adjQuad(dframe, f, fprime, f2prime, w = input$weight)
      title = "Correction: Quadratic"
    } else if(corr==3){
      dframeAdj1 <- adjLinear(dframe, f, fprime, f2prime, w = input$weight)
      title = "Correction: Linear"
    } else if(corr==2){
      dframeAdj1 <- adjGeom(dframe, f, fprime, f2prime, w = input$weight)
      title = "Correction: Trigonometric"
    } else {
      dframeAdj1 <- adjNone(dframe, f, fprime, f2prime, w = input$weight)
    ...
    p <- qplot(x=x, xend=x, y=seg.ystart, yend=seg.yend, geom="segment", data=dplot) +
      theme_bw() + coord_equal(ratio=1) + facet_grid(type~adj) +
      scale_x_continuous(breaks=seq(-pi, pi, by=pi/2), 
                         labels=c(expression(-pi), expression(paste(-pi, "/2")), 0, 
                                  expression(paste(pi,"/2")), expression(pi))) +
      xlab("x") + ylab("y")
